...
if __name__ == "__main__":
    ...
    for net_nr in range(10, nets):
        print('Net nr', net_nr)
        net_trained.load_state_dict(torch.load('../nets/MNIST_MLP(20, 10)_trained_%d.pt' % net_nr))
        net_trained.eval()
        net_untrained.load_state_dict(torch.load('../nets/MNIST_MLP(20, 10)_untrained_%d.pt' % net_nr))
        net_untrained.eval()
        ...
        df_acc_class = pd.DataFrame(accuracies_class)

        path_acc = '../results/acc_net%d.xlsx' % net_nr
        path_acc_class = '../results/acc_class_net%d.xlsx' % net_nr

        df_acc.to_excel(path_acc, index=False)
        df_acc_class.to_excel(path_acc_class, index=False)
