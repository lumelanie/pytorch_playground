...
class Net(nn.Module):
    ...
    def train_net(self, criterion, optimizer, trainloader, epochs, device):
        # save untrained net
        if save:
            torch.save(net.state_dict(), '../nets/MNIST_MLP(20, 10)_untrained_%d.pt' % net_nr)
        # train the net
        log_interval = 10
        ...
        if save:
            # save trained net
            torch.save(net.state_dict(), '../nets/MNIST_MLP(20, 10)_trained_%d.pt' % net_nr)
...
if __name__ == "__main__":
    ...
    save = False
    test = True
    save_acc = True # only works if test = True
    nets = 20
    ...
    if save_acc:
        accuracies_class = np.zeros((10, nets))
    for net_nr in range(nets):
        # setting rng seed for reproduceaility
        np.random.seed(net_nr)
        torch.manual_seed(net_nr)
        torch.cuda.manual_seed_all(net_nr)
        ...
        if train:
            ...
            net.load_state_dict(torch.load('../nets/MNIST_MLP(20, 10)_trained_%d.pt' % net_nr))
            net.eval()

        if test:
            acc, correct_labels, acc_class, class_labels = net.test_net(criterion, testloader, device)
            if save_acc:
                accuracies_class[:, net_nr] = acc_class
            print(acc)
    ...
    if save_acc:
        df = pd.DataFrame(accuracies_class)
        df.to_excel('../results/accuracies_class.xlsx')